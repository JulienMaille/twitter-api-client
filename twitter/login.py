    ...
    try:
        ...
        for task in info.get('subtasks', []):
            ...
            if subtask_id == 'DenyLoginSubtask' or subtask_id == 'ArkoseLogin':
                print(f'[{RED}error{RESET}] failed to login, subtask_id: {subtask_id}')
        ...
        print(f'[{RED}error{RESET}] failed to update token at {caller_name}\n{e}')
    return client
...
def flow_start(client: Client) -> Client:
    return update_token(client, 'flow_token', TASK_URL, params={'flow_name': 'login'}, json={
        "input_flow_data": {
            "flow_context": {
                "debug_overrides": {},
...
def login(email: str, username: str, password: str, **kwargs) -> Client:
    ...
    if not client or client.cookies.get('flow_errors') == 'true':
        raise Exception(f'[{RED}error{RESET}] {username} login failed')
    return client
